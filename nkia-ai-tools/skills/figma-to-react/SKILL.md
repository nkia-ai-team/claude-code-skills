...
### 토큰 마이그레이션 (-m)
    ...
    # 기존 컴포넌트의 토큰 체계를 마이그레이션
    # 구 토큰 → 신 토큰 자동 교체 + Figma 디자인 diff 반영
    /figma-to-react https://...?node-id=1234-5678 -m
...
### 컴포넌트 단위
    ...
    → 5. 디자인 토큰 매칭 → 6. React 컴포넌트 생성 → 6.5. 인터랙션 연결
    → 7. Storybook 스토리 (개발 참고용) → 화면 통합 → 8. QA Phase (dev 서버에서 서브에이전트 3개 병렬 검증)
    → 8.5. Interaction Review (추론된 인터랙션 사용자 확인)
...
### 화면 수준 — 토큰 마이그레이션 (-m)
    ...
    1. Figma MCP 추출 → 2. 기존 코드 탐색
    → 3. 토큰 마이그레이션 맵 구성 (정적 매핑 + HEX 매칭 + Figma 보완)
    → 4. Figma ↔ 코드 디자인 diff → 5. 변경 계획 사용자 확인
    → 6. 변경 적용 (토큰 교체 + 디자인 diff) → 7. 결과 요약
...
- 증분 모드: `-a`는 새 컴포넌트만 빌드+삽입, `-u`는 Figma↔코드 diff 후 변경분만 반영, `-m`은 구 토큰 → 신 토큰 전수 교체 + Figma 디자인 diff 반영 (비즈니스 로직 불변). 기존 코드를 처음부터 다시 쓰지 않는다
