...
### A-6. QA Phase (코드 변경이 있는 경우만)

신규 빌드된 컴포넌트를 화면에 통합한 후, dev 서버에서 QA를 수행한다.
Step 8(QA Phase)과 동일한 절차 — QA-기능/QA-시각적/QA-토큰 3개 에이전트 병렬 실행.

코드 변경이 없으면 A-9 결과 요약으로 건너뛴다.

### A-7. Storybook 스토리 업데이트 (코드 변경이 있는 경우만)

신규 빌드된 컴포넌트의 Storybook 스토리를 생성/업데이트한다.
Step 7(Storybook 스토리 생성)과 동일한 절차.

### A-8. Code Connect 업데이트 (코드 변경이 있는 경우만)

신규 빌드된 컴포넌트의 .figma.tsx 매핑을 생성/업데이트한다.
Step 9(Code Connect 매핑 등록)와 동일한 절차.
...
### U-6. QA Phase (코드 변경이 있는 경우만)

U-5에서 실제 코드 변경이 발생한 경우에만 수행한다.
변경 적용 후 dev 서버에서 QA를 수행한다.
Step 8(QA Phase)과 동일한 절차 — QA-기능/QA-시각적/QA-토큰 3개 에이전트 병렬 실행.

코드 변경이 없으면 (diff만 확인하고 변경 사항 없음) U-9 결과 요약으로 건너뛴다.

### U-7. Storybook 스토리 업데이트 (코드 변경이 있는 경우만)

변경된 컴포넌트의 .stories.tsx를 업데이트한다.
- 신규 컴포넌트: Step 7(Storybook 스토리 생성) 절차로 생성
- 기존 컴포넌트: props/variant 변경이 있으면 스토리도 함께 업데이트
- 토큰만 변경된 경우: 스토리에 구 토큰 참조가 있으면 교체

### U-8. Code Connect 업데이트 (코드 변경이 있는 경우만)

변경된 컴포넌트의 .figma.tsx 매핑을 업데이트한다.
- 신규 컴포넌트: Step 9(Code Connect 매핑 등록) 절차로 생성
- 기존 컴포넌트: props/variant 변경이 있으면 매핑도 함께 업데이트
- 토큰만 변경된 경우: Code Connect 매핑에 영향 없으므로 건너뛰기
...
### M-5. QA Phase (코드 변경이 있는 경우만)

토큰 치환으로 코드가 변경된 경우, dev 서버에서 QA를 수행한다.
Step 8(QA Phase)과 동일한 절차.

코드 변경이 없으면 (매핑 실패만 있어서 실제 치환 0건) 건너뛴다.

### M-6. Storybook 스토리 업데이트 (코드 변경이 있는 경우만)

토큰이 교체된 컴포넌트의 .stories.tsx에서도 구 토큰 참조를 신 토큰으로 교체한다.
코드 변경이 없으면 건너뛴다.

### M-7. Code Connect 업데이트 (코드 변경이 있는 경우만)

토큰 치환은 Tailwind 클래스명 변경이므로 Code Connect 매핑(props/variant 구조)에는 보통 영향 없다.
props/variant 이름이 변경된 경우에만 .figma.tsx 매핑을 업데이트한다.
코드 변경이 없으면 건너뛴다.
