# `sanity_check` — APM standby agent heartbeat sanity check
...
5회 (5분) 후 미도달 → 사용자에게 원인 후보 표시 + manifest 점검 권고. `polestar10_register` 진입 X.
...
- **blocking**: collector_host 변경 필요 → bootstrap.yaml 수정 + ansible 재실행 권고 (`ansible_deploy` 부터 resume)
