...
| 헤더 행 | 4행 (보고자, 업무구분, 업무, 업무 내용, 투입시간, 업무 구분, 업무) |
| 데이터 시작 행 | 5행~ |
...
해당 탭의 A열을 읽어 보고자 이름을 찾습니다:
    ...
    gws sheets spreadsheets values get \
      --params '{"spreadsheetId": "{spreadsheetId}", "range": "{tabName}!A1:A30"}'
...
**이름을 찾을 수 없는 경우:**
    ...
    ERROR: A열에서 "{reporterName}"을(를) 찾을 수 없습니다.
    config의 reporterName이 시트에 기재된 이름과 일치하는지 확인해주세요.
    A열에 있는 이름 목록: {발견된 이름들}
