...
이슈 수집에 앞서 현재 활성 cycle을 파악합니다. **이 단계가 완료된 후** 2.1의 이슈 조회를 시작합니다.
    ...
    mcp__linear__list_cycles(team: myTeam, active: true)
    → 현재 활성 cycle의 ID를 currentCycle로 설정
    → 직전 cycle ID를 prevCycle로 설정 (cycleNumber - 1)

> API가 cycle 목록을 반환하지 않으면: 첫 번째 이슈의 `cycle.id`에서 currentCycle을 추출합니다.
...
| B | 직전 cycle Done 이슈 | `mcp__linear__list_issues(assignee: "me", cycle: prevCycle, state: "Done")` | 조회 후 **로컬**에서 `completedAt >= weekStart`로 필터 |
...
- 조회 B: 직전 cycle에서 **이번 주에 완료된** 이슈 cross-cycle 추적 (`completedAt >= weekStart` 로컬 필터 필수)
...
| **in_progress_issues** | cycle == currentCycle AND state == "In Progress" | 금주 실적 — 진행 중 항목 + 차주 계획 |
