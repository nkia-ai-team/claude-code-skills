...
- **RestClient 미지원**: Spring 6.1 이후 표준이 된 `RestClient` 인터페이스에 대한 계측이 없어 호출이 집계에서 누락됩니다. `WebClient` / `RestTemplate` 로 대체하거나, 커스텀 계측을 추가해야 합니다.
- **`@XxxMapping` value 필수**: `@GetMapping`, `@PostMapping` 등 요청 매핑 애노테이션의 `value` (또는 `path`) 속성이 비어 있으면 WPM 이 엔드포인트를 인식하지 못해 metric 이 생성되지 않습니다. `@GetMapping("")` 처럼 명시적 빈 문자열이라도 넣어야 인식됩니다.
